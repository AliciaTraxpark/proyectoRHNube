...
function calendario() {
    ...
    $.ajax({
        ...
        success: function (data) {
            $("#fechaEnviF").val(data);
            $("#fechaHasta").text(
                moment(data).subtract(1, "day").format("DD/MM/YYYY")
            );
        },
        error: function () {},
    });
    ...
    var configuracionCalendario = {
        ...
        eventClick: function (info) {
            ...
            if (info.event.textColor == "#504545") {
        ...
        customButtons: {
            nuevoAño: {
                ...
                click: function () {
                    ...
                    $("#textoNuevoAño").val(
                        "¿Añadir año " + añoEnviado + " al calendario actual?"
                    );
            ...
            Asignar: {
                ...
                click: function () {
                    $("#nombreEmpleado").load(
                        location.href + " #nombreEmpleado>*"
                    );
                    var nombreca = $(
                        'select[id="selectCalendario"] option:selected'
                    ).text();
                ...
                success: function (data) {
                    $.each(data, function (index, value) {
                        ...
                        if (value.laborable == 0) {
                            var element = $(
                                "div.fc-bg > table > tbody > tr > td.fc-day.fc-widget-content[data-date]"
                            );
...
function registrarDdescanso() {
    ...
    $.ajax({
        type: "POST",
        url: "/eventos_calendario/storeDescanso",
        data: {
            title,
            color,
            textColor,
        ...
        success: function (data) {
            if (data == 1) {
                bootbox.confirm({
                    ...
                    message:
                        "¿Agregar los cambios a los empleados asignados a este calendario?",
                    buttons: {
                        confirm: {
                            label: "Si",
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (a.diff(b, "days") > 1) {
                $(
                    "div.fc-bg > table > tbody > tr > td.fc-day.fc-widget-content[data-date='" +
                        moment(a).subtract(1, "day").format("YYYY-MM-DD") +
                        "']"
...
$("#selectEmpresarial").change(function (e) {
    ...
    textSelec = $(
        'select[name="selectEmpresarial"] option:selected:last'
    ).text();
    textSelec2 = $(
        'select[name="selectEmpresarial"] option:selected:last'
    ).text();
        ...
        ajax: {
            ...
            complete: function (data) {
                ...
                $.each(dataD, function (index, value1) {
                    ...
                    $("#nombreEmpleado")
                        .find('option[value="' + value1.emple_id + '"]')
                        .remove();
            ...
            $.each(data, function (key, item) {
                if (item[0].calendario_nombre == null) {
                    $("#empleadosSele>tbody").append(
                        "<tr id='" +
                            item[0].emple_id +
                            "' ><td> <input type='checkbox' class='ml-4' name='idEmSelecto' id='idEmSelecto' data-id='" +
                            item[0].emple_id +
                            "'>&nbsp;&nbsp; <a style='cursor: pointer' class='borrar' onclick='quitars(" +
                            item[0].emple_id +
                            ")'  ><img style='margin-bottom: 6px;' src='admin/images/delete.svg' height='15'></a> </td>  <td >" +
                            item[0].perso_nombre +
                            "</td> <td >" +
                            item[0].perso_apPaterno +
                            "</td><td>" +
                            item[0].perso_apMaterno +
                            "</td><td>Ninguno</td></tr>"
                    );
                } else {
                    $("#empleadosSele>tbody").append(
                        "<tr id='" +
                            ...
                            "' ><td> <input type='checkbox' class='ml-4' name='idEmSelecto' id='idEmSelecto' data-id='" +
                            item[0].emple_id +
                            "'>&nbsp;&nbsp; <a style='cursor: pointer' onclick='quitars(" +
                            item[0].emple_id +
                            ")' class='borrar' ><img style='margin-bottom: 6px;' src='admin/images/delete.svg' height='15'></a> </td>  <td >" +
                            item[0].perso_nombre +
                            "</td> <td >" +
                            item[0].perso_apPaterno +
                            "</td><td>" +
...
$("#selectClonar").change(function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            $.each(data, function (index, value) {
                $("#añosCalen").append(
                    " &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type='checkbox'  class='form-check-input' id='añosCheck' name='añosCheck' value='" +
                        value +
                        "'>" +
                        "<label class='form-check-label' for='' style='margin-top: 2px;' >" +
                        value +
                        "</label>&nbsp;&nbsp;&nbsp;"
                );
            });
...
function agregarMFeriado() {
 ...
 /* **************************************************** */
 /*** BOTON NUEVO DESCANSO*/
function agregarMDescanso() {
